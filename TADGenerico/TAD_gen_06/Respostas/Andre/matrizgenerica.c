...
void DestroiMatrizGenerica(tMatrizGenerica *mat)
{
    ...
    for (int i = 0; i < (mat->lins * mat->cols); i++)
    {
...
void *ObtemElementoMatrizGenerica(tMatrizGenerica *mat, int linha, int coluna)
{
    ...
    return mat->dados[mat->cols * linha + coluna];
}
...
void AtribuiElementoMatrizGenerica(tMatrizGenerica *mat, int linha, int coluna, void *elem)
{
    ...
    mat->dados[mat->cols * linha + coluna] = elem;
}
...
void ImprimirMatrizGenerica(tMatrizGenerica *mat, void(imprime_elemento(void *)))
{
    ...
    for (int i = 0; i < mat->lins; i++)
    {
        ...
        for (int j = 0; j < mat->cols; j++)
        {
            ...
            imprime_elemento(mat->dados[mat->cols * i + j]);
        }
    }
}
...
tMatrizGenerica *MatrizTransposta(tMatrizGenerica *mat)
{
    ...
    tMatrizGenerica *m2 = CriaMatrizGenerica(mat->cols, mat->lins, mat->bytes);
    ...
    for (int i = 0; i < mat->lins; i++)
    {
        ...
        for (int j = 0; j < mat->cols; j++)
        {
            ...
            m2->dados[m2->lins * j + i] = mat->dados[mat->cols * i + j];
        }
    }
...

{
    ...
    tMatrizGenerica *m3 = CriaMatrizGenerica(mat1->lins, mat2->cols, numByteTarget);
    ...
    for (int i = 0; i < mat1->lins; i++)
    {

        for (int j = 0; j < mat2->cols; j++)
        {

            acumulador = calloc(1, numByteTarget);

            for (int k = 0; k < mat1->cols; k++)
            {

                d1 = ObtemElementoMatrizGenerica(mat1, i, k);
                d2 = ObtemElementoMatrizGenerica(mat2, k, j);

                produto = multi_elem(d1, d2);   
                void *novoacumulador = soma_elem(acumulador, produto);
            ...
            AtribuiElementoMatrizGenerica(m3, i, j, acumulador);
            free(acumulador);
        }
    }
